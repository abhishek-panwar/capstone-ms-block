...
public class Project {
	...
	/** 
	 * checks if node with given id is present in the network 
	 * @param id
	 * @return
	 */
	...
	/**
	 * a new block is taken care after it is created 
	 * @param block
	 * @param obm
	 */
	...
	/**
	 * broadcast the block to the overlay
	 * @param block
	 * @param obm
	 */
	public void broadcastGlobally(OverlayBlock block, OverlayNode obm)
	{
		for (OverlayNode node: this.overlayNodes) 
		{
			if(node!=obm)
				node.verifyBlock(block);
		}
	}
	
	/**
	 * a new transaction is taken care for verification purposes
	 * @param transaction
	 * @return
	 */
	...
	/**
	 * broadcast the transaction to overlay
	 * @param transaction
	 * @return
	 */
